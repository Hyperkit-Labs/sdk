...
import { X, Info, AlertCircle, XCircle, CheckCircle, Wallet, Copy, LogOut, ArrowUpDown, ArrowRight, TrendingUp, Coins, Clock, Droplets } from 'lucide-react';
...
const DEFAULT_NETWORK = "metis-hyperion-testnet";

const HyperkitContext = createContext(null);
function HyperkitProvider({ children, defaultNetwork = DEFAULT_NETWORK }) {
    ...
    const isCorrectNetwork = (networkKey = defaultNetwork) => {
        if (!wallet.chainId)
            return false;
        const network = NETWORKS[networkKey];
    ...
    return (jsxRuntimeExports.jsx(HyperkitContext.Provider, { value: value, children: children }));
}
function useWallet() {
...
function Container({ children, width = '100%', height = 'auto', scale = 1, theme = 'light', className = '', style = {} }) {
    ...
    return (jsxRuntimeExports.jsx("div", { className: `hyperkit-container ${themeClass} ${className}`, style: containerStyle, "data-theme": theme, children: children }));
}

function Alert({ type, message, isVisible, onClose, autoClose = true, autoCloseDuration = 5000, absolute = false, position = 'top' }) {
    React.useEffect(() => {
    ...
    const getIcon = () => {
        switch (type) {
            case 'success':
                return jsxRuntimeExports.jsx(CheckCircle, { className: "w-5 h-5" });
            case 'error':
                return jsxRuntimeExports.jsx(XCircle, { className: "w-5 h-5" });
            case 'warning':
                return jsxRuntimeExports.jsx(AlertCircle, { className: "w-5 h-5" });
            case 'info':
                return jsxRuntimeExports.jsx(Info, { className: "w-5 h-5" });
        }
    };
    const getClassNames = () => {
    ...
    return (jsxRuntimeExports.jsx("div", { className: getClassNames(), children: jsxRuntimeExports.jsxs("div", { className: "hyperkit-alert-content", children: [jsxRuntimeExports.jsx("div", { className: "hyperkit-alert-icon", children: getIcon() }), jsxRuntimeExports.jsx("div", { className: "hyperkit-alert-message", children: message }), onClose && (jsxRuntimeExports.jsx("button", { onClick: onClose, className: "hyperkit-alert-close", "aria-label": "Close alert", children: jsxRuntimeExports.jsx(X, { className: "w-4 h-4" }) }))] }) }));
}
function useAlert() {
...
function ConnectWallet({ onSuccess, onError, onDisconnect, showDropdown = true, ...containerProps }) {
    ...
    if (!wallet.isConnected) {
        return (jsxRuntimeExports.jsx(Container, { ...containerProps, children: jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-4", children: [jsxRuntimeExports.jsxs("button", { onClick: handleConnect, disabled: wallet.isLoading, className: "connectButton", children: [jsxRuntimeExports.jsx(Wallet, { className: "w-5 h-5" }), wallet.isLoading ? 'Connecting...' : 'Connect Wallet'] }), jsxRuntimeExports.jsx(Alert, { ...alertProps })] }) }));
    }
    if (!showDropdown) {
        return (jsxRuntimeExports.jsx(Container, { ...containerProps, children: jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-4", children: [jsxRuntimeExports.jsxs("div", { className: "connectButton actionButton", children: [jsxRuntimeExports.jsx(Wallet, { className: "w-5 h-5" }), formatAddress(wallet.account)] }), jsxRuntimeExports.jsx(Alert, { ...alertProps })] }) }));
    }
    return (jsxRuntimeExports.jsx(Container, { ...containerProps, children: jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-4", children: [jsxRuntimeExports.jsxs("div", { className: "wallet-dropdown", ref: dropdownRef, children: [jsxRuntimeExports.jsx("button", { onClick: () => setIsDropdownOpen(!isDropdownOpen), className: "connectButton actionButton justify-between", children: jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx(Wallet, { className: "w-5 h-5" }), formatAddress(wallet.account)] }) }), isDropdownOpen && (jsxRuntimeExports.jsxs("div", { className: "wallet-dropdown-content", children: [jsxRuntimeExports.jsxs("div", { className: "wallet-dropdown-item", onClick: copyAddress, children: [copied ? (jsxRuntimeExports.jsx(CheckCircle, { className: "w-4 h-4 text-green-500" })) : (jsxRuntimeExports.jsx(Copy, { className: "w-4 h-4" })), copied ? 'Copied!' : 'Copy Address'] }), jsxRuntimeExports.jsxs("div", { className: "wallet-dropdown-item", onClick: handleDisconnect, children: [jsxRuntimeExports.jsx(LogOut, { className: "w-4 h-4" }), "Disconnect"] })] }))] }), jsxRuntimeExports.jsx(Alert, { ...alertProps })] }) }));
}
...
// Staking ABI (continues with your provided ABI)
const STAKING_ABI = [
    {
        "inputs": [
            {
...
// Bridge ABI (continues with your provided ABI - truncated for brevity)
const BRIDGE_ABI = [
    {
        "inputs": [
            {
    ...
    // ... continuing with the rest of the bridge ABI
    {
        "inputs": [
            {
                "internalType": "address",
...
// Faucet ABI (continues with your provided ABI - truncated for brevity)
const FAUCET_ABI = [
    {
        "inputs": [
            {
...
class BlockchainActions {
    ...
    async approveToken(tokenSymbol, spenderAddress, amount) {
        ...
        const decimals = TOKENS[tokenSymbol].decimals;
        const amountWei = parseUnits$1(amount, decimals);
        return await contract.approve(spenderAddress, amountWei);
    }
    async getAllowance(tokenSymbol, ownerAddress, spenderAddress) {
        const tokenAddress = TOKEN_ADDRESSES[tokenSymbol];
        const contract = new Contract(tokenAddress, ERC20_ABI, this.provider);
    ...
    async getSwapQuote(tokenIn, tokenOut, amountIn) {
        const contract = new Contract(CONTRACT_ADDRESSES.LIQUIDITY_POOL, LIQUIDITY_POOL_ABI, this.provider);
        ...
        const amountInWei = parseUnits$1(amountIn, decimalsIn);
        const amountOutWei = await contract.getAmountOut(amountInWei, tokenInAddress, tokenOutAddress);
        const rawOutputAmount = formatUnits(amountOutWei, decimalsOut);
    ...
    async swapTokens(tokenIn, tokenOut, amountIn, expectedAmountOut, recipient, slippageTolerance) {
        const contract = new Contract(CONTRACT_ADDRESSES.LIQUIDITY_POOL, LIQUIDITY_POOL_ABI, this.signer);
        const tokenInAddress = TOKEN_ADDRESSES[tokenIn];
        const tokenOutAddress = TOKEN_ADDRESSES[tokenOut];
        const decimalsIn = TOKENS[tokenIn].decimals;
        const decimalsOut = TOKENS[tokenOut].decimals;
        ...
        const minAmountOutWei = parseUnits$1(minAmountOut, decimalsOut);
    ...
    async executeSwap(tokenIn, tokenOut, amountIn, minAmountOut) {
        ...
        const amountInWei = parseUnits$1(truncatedAmountIn, decimalsIn);
        const minAmountOutWei = parseUnits$1(truncatedMinAmountOut, decimalsOut);
        return await contract.swap(tokenInAddress, tokenOutAddress, amountInWei, minAmountOutWei);
    }
    // Staking Actions
    async getStakedBalance(userAddress) {
        const contract = new Contract(CONTRACT_ADDRESSES.STAKING_REWARDS, STAKING_ABI, this.provider);
        const balance = await contract.getStakedBalance(userAddress);
        return formatUnits(balance, 6); // Assuming 18 decimals
    }
    async getPendingRewards(userAddress) {
        const contract = new Contract(CONTRACT_ADDRESSES.STAKING_REWARDS, STAKING_ABI, this.provider);
        const rewards = await contract.getPendingReward(userAddress);
        return formatUnits(rewards, 6);
    }
    async stakeTokens(amount) {
        const contract = new Contract(CONTRACT_ADDRESSES.STAKING_REWARDS, STAKING_ABI, this.signer);
        const amountWei = parseUnits$1(amount, 6);
    ...
    // Bridge Actions
    async bridgeDeposit(tokenSymbol, amount, destinationChainId, destinationAddress) {
        // Create contract instance with explicit typing
        const contract = new Contract(CONTRACT_ADDRESSES.BRIDGE, BRIDGE_ABI, this.signer); // Temporary type assertion to bypass potential ABI issues
        const tokenAddress = TOKEN_ADDRESSES[tokenSymbol];
        const decimals = TOKENS[tokenSymbol].decimals;
        const amountWei = parseUnits$1(amount, decimals);
        // Send native tMETIS as value for the gas fee (0.001 ETH to match your working script)
        const bridgeFee = parseEther("0.001");
        ...
        try {
            const tx = await contract.deposit(tokenAddress, amountWei, destinationChainId, destinationAddress, { value: bridgeFee });
    ...
    async getBridgeStats() {
        const contract = new Contract(CONTRACT_ADDRESSES.BRIDGE, BRIDGE_ABI, this.provider);
        ...
        return {
            ...
            currentFee: formatEther(stats.currentFee),
            currentTimeout: stats.currentTimeout.toString()
        };
    }
    // Faucet Actions
    async getSupportedFaucetTokens() {
        const contract = new Contract(CONTRACT_ADDRESSES.FAUCET, FAUCET_ABI, this.provider);
        return await contract.getSupportedTokens();
    }
    async getFaucetTokenInfo(tokenAddress) {
        const contract = new Contract(CONTRACT_ADDRESSES.FAUCET, FAUCET_ABI, this.provider);
        ...
        return {
            ...
            dripAmount: formatUnits(info.dripAmount, faucetDecimals),
            maxBalance: formatUnits(info.maxBalance, faucetDecimals),
            isActive: info.isActive,
            faucetBalance: formatUnits(info.faucetBalance, faucetDecimals)
        };
    }
    async getUserFaucetInfo(userAddress, tokenAddress) {
        const contract = new Contract(CONTRACT_ADDRESSES.FAUCET, FAUCET_ABI, this.provider);
        const info = await contract.getUserTokenInfo(userAddress, tokenAddress);
        return {
            lastDripTime: Number(info.lastDripTime),
            totalDripped: formatUnits(info.totalDripped, 18),
            tokenDripped: formatUnits(info.tokenDripped, 18),
            canDrip: info.canDrip,
            timeUntilNextDrip: Number(info.timeUntilNextDrip)
        };
    }
    async requestFaucetDrip(tokenAddress) {
        const contract = new Contract(CONTRACT_ADDRESSES.FAUCET, FAUCET_ABI, this.signer);
        return await contract.drip(tokenAddress);
    }
    async requestAllFaucetDrips() {
        const contract = new Contract(CONTRACT_ADDRESSES.FAUCET, FAUCET_ABI, this.signer);
...
function createBlockchainActions(provider, signer) {
    return new BlockchainActions(provider, signer);
}

function Bridge({ onSuccess, onError, supportedTokens = ['USDT', 'USDC'], supportedNetworks = ['metis-hyperion-testnet', 'metisSepolia'], ...containerProps }) {
    ...
    const loadBalances = async () => {
        if (!blockchainActions || !wallet.account)
            return;
        try {
            const newBalances = {};
    ...
    const handleBridge = async () => {
        ...
        try {
            ...
            const allowance = await blockchainActions.getAllowance(selectedToken, wallet.account, CONTRACT_ADDRESSES.BRIDGE // Approve the bridge contract, not the token
            );
            if (parseFloat(allowance) < amountNum) {
                showAlert('info', 'Approving token...', { absolute: true, position: 'top' });
                const approveTx = await blockchainActions.approveToken(selectedToken, CONTRACT_ADDRESSES.BRIDGE, // Approve the bridge contract, not the token
                amount);
            ...
            const bridgeTx = await blockchainActions.bridgeDeposit(selectedToken, amount, destinationChainId, wallet.account);
    ...
    return (jsxRuntimeExports.jsx(Container, { ...containerProps, children: jsxRuntimeExports.jsxs("div", { className: "card", children: [jsxRuntimeExports.jsxs("div", { className: "cardHeader", children: [jsxRuntimeExports.jsx("h2", { className: "cardTitle", children: "Bridge" }), jsxRuntimeExports.jsx("p", { className: "cardDescription", children: "Transfer tokens across networks" })] }), jsxRuntimeExports.jsxs("div", { className: "cardContent", children: [jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "From Network" }), jsxRuntimeExports.jsx("select", { value: fromNetwork, onChange: (e) => setFromNetwork(e.target.value), disabled: isProcessing, className: "sel_field", children: supportedNetworks.map(network => (jsxRuntimeExports.jsx("option", { value: network, children: NETWORKS[network]?.chainName || network }, network))) })] }), jsxRuntimeExports.jsx("div", { className: "swapButtonContainer", children: jsxRuntimeExports.jsx("button", { type: "button", onClick: handleSwapNetworks, disabled: isProcessing, className: "swapDirectionButton", "aria-label": "Swap networks", children: jsxRuntimeExports.jsx(ArrowUpDown, { className: "w-5 h-5" }) }) }), jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "To Network" }), jsxRuntimeExports.jsx("select", { value: toNetwork, onChange: (e) => setToNetwork(e.target.value), disabled: isProcessing, className: "sel_field", children: supportedNetworks.map(network => (jsxRuntimeExports.jsx("option", { value: network, children: NETWORKS[network]?.chainName || network }, network))) })] }), jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "Token" }), jsxRuntimeExports.jsx("select", { value: selectedToken, onChange: (e) => setSelectedToken(e.target.value), disabled: isProcessing, className: "sel_field", children: supportedTokens.map(token => (jsxRuntimeExports.jsxs("option", { value: token, children: [TOKENS[token].name, " (", token, ")"] }, token))) })] }), jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsxs("label", { className: "label", children: ["Amount", wallet.isConnected && balances[selectedToken] && (jsxRuntimeExports.jsxs("span", { className: "text-sm text-gray-500 ml-2", children: ["Balance: ", parseFloat(balances[selectedToken]).toFixed(4), " ", selectedToken] }))] }), jsxRuntimeExports.jsx("input", { type: "text", placeholder: "0.0", value: amount, onChange: (e) => setAmount(e.target.value), disabled: isProcessing, className: "txt_field" })] }), wallet.isConnected && !isCorrectNetwork(fromNetwork) && (jsxRuntimeExports.jsx("div", { className: "hyperkit-alert hyperkit-alert-warning", children: jsxRuntimeExports.jsx("div", { className: "hyperkit-alert-content", children: jsxRuntimeExports.jsxs("div", { className: "hyperkit-alert-message", children: ["Please switch to ", NETWORKS[fromNetwork]?.chainName, " to continue"] }) }) }))] }), jsxRuntimeExports.jsx("div", { className: "cardFooter", children: !wallet.isConnected ? (jsxRuntimeExports.jsx("div", { className: "text-center text-gray-500", children: "Please connect your wallet to use the bridge" })) : (jsxRuntimeExports.jsx("button", { onClick: handleBridge, disabled: isProcessing ||
                            !isValidAmount() ||
                            fromNetwork === toNetwork ||
                            !isCorrectNetwork(fromNetwork), className: "actionButton", children: isProcessing ? (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx("div", { className: "loading-spinner" }), "Processing..."] })) : !isCorrectNetwork(fromNetwork) ? (`Switch to ${NETWORKS[fromNetwork]?.chainName}`) : (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: ["Bridge to ", NETWORKS[toNetwork]?.chainName, jsxRuntimeExports.jsx(ArrowRight, { className: "w-4 h-4" })] })) })) }), jsxRuntimeExports.jsx(Alert, { ...alertProps })] }) }));
}

function Swap({ onSuccess, onError, supportedTokens = ['USDT', 'USDC', 'WETH', 'DAI'], defaultFromToken = 'USDT', defaultToToken = 'USDC', ...containerProps }) {
    ...
    const handleSwap = async () => {
        ...
        try {
            ...
            // Step 1: Check and approve token
            const allowance = await blockchainActions.getAllowance(fromToken, wallet.account, CONTRACT_ADDRESSES.LIQUIDITY_POOL // This should be swap router address
            );
            if (parseFloat(allowance) < fromAmountNum) {
                showAlert('info', 'Approving token...', { absolute: true, position: 'top' });
                const approveTx = await blockchainActions.approveToken(fromToken, CONTRACT_ADDRESSES.LIQUIDITY_POOL, fromAmount);
                showAlert('info', `Approval submitted: ${approveTx.hash.slice(0, 10)}...`, { absolute: true, position: 'top' });
                await approveTx.wait();
                showAlert('success', 'Token approved successfully', { absolute: true, position: 'top' });
            }
            ...
            const swapTx = await blockchainActions.swapTokens(fromToken, toToken, fromAmount, toAmount, wallet.account, parseFloat(slippage));
    ...
    return (jsxRuntimeExports.jsx(Container, { ...containerProps, children: jsxRuntimeExports.jsxs("div", { className: "card", children: [jsxRuntimeExports.jsxs("div", { className: "cardHeader", children: [jsxRuntimeExports.jsx("h2", { className: "cardTitle", children: "Swap" }), jsxRuntimeExports.jsx("p", { className: "cardDescription", children: "Exchange tokens instantly" })] }), jsxRuntimeExports.jsxs("div", { className: "cardContent", children: [jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center mb-2", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "From" }), wallet.isConnected && balances[fromToken] && (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 text-sm text-gray-500", children: [jsxRuntimeExports.jsxs("span", { children: ["Balance: ", formatBalance(balances[fromToken])] }), jsxRuntimeExports.jsx("button", { type: "button", onClick: handleMaxClick, disabled: isProcessing, className: "text-blue-500 hover:text-blue-600 font-medium", children: "MAX" })] }))] }), jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [jsxRuntimeExports.jsx("input", { type: "text", placeholder: "0.0", value: fromAmount, onChange: (e) => setFromAmount(e.target.value), disabled: isProcessing, className: "txt_field flex-1" }), jsxRuntimeExports.jsx("select", { value: fromToken, onChange: (e) => setFromToken(e.target.value), disabled: isProcessing, className: "sel_field w-32", children: supportedTokens.map(token => (jsxRuntimeExports.jsx("option", { value: token, children: token }, token))) })] })] }), jsxRuntimeExports.jsx("div", { className: "swapButtonContainer", children: jsxRuntimeExports.jsx("button", { type: "button", onClick: handleSwapTokens, disabled: isProcessing, className: "swapDirectionButton", "aria-label": "Swap tokens", children: jsxRuntimeExports.jsx(ArrowUpDown, { className: "w-5 h-5" }) }) }), jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center mb-2", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "To" }), wallet.isConnected && balances[toToken] && (jsxRuntimeExports.jsxs("span", { className: "text-sm text-gray-500", children: ["Balance: ", formatBalance(balances[toToken])] }))] }), jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [jsxRuntimeExports.jsx("input", { type: "text", placeholder: "0.0", value: toAmount, readOnly: true, className: "txt_field flex-1 bg-gray-50" }), jsxRuntimeExports.jsx("select", { value: toToken, onChange: (e) => setToToken(e.target.value), disabled: isProcessing, className: "sel_field w-32", children: supportedTokens.map(token => (jsxRuntimeExports.jsx("option", { value: token, children: token }, token))) })] })] }), jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "Slippage Tolerance (%)" }), jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [['0.1', '0.5', '1.0'].map(value => (jsxRuntimeExports.jsxs("button", { type: "button", onClick: () => setSlippage(value), disabled: isProcessing, className: `px-3 py-1 rounded text-sm border ${slippage === value
                                                ? 'bg-blue-500 text-white border-blue-500'
                                                : 'bg-white text-gray-700 border-gray-300 hover:border-blue-300'}`, children: [value, "%"] }, value))), jsxRuntimeExports.jsx("input", { type: "text", placeholder: "Custom", value: slippage, onChange: (e) => setSlippage(e.target.value), disabled: isProcessing, className: "txt_field flex-1 text-sm" })] })] })] }), jsxRuntimeExports.jsx("div", { className: "cardFooter", children: !wallet.isConnected ? (jsxRuntimeExports.jsx("div", { className: "text-center text-gray-500", children: "Please connect your wallet to use the swap" })) : (jsxRuntimeExports.jsx("button", { onClick: handleSwap, disabled: isProcessing ||
                            !isValidAmount() ||
                            fromToken === toToken ||
                            !toAmount ||
                            parseFloat(toAmount) <= 0, className: "actionButton", children: isProcessing ? (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx("div", { className: "loading-spinner" }), "Processing..."] })) : (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx(TrendingUp, { className: "w-4 h-4" }), "Swap ", fromToken, " for ", toToken] })) })) }), jsxRuntimeExports.jsx(Alert, { ...alertProps })] }) }));
}

function Staking({ onSuccess, onError, supportedTokens = ['USDT', 'USDC'], defaultStakeToken = 'USDT', ...containerProps }) {
    const { wallet } = useWallet();
    const { showAlert, alertProps } = useAlert();
    const [selectedToken, setSelectedToken] = useState(defaultStakeToken);
    ...
    const handleStake = async () => {
        ...
        try {
            ...
            const allowance = await blockchainActions.getAllowance(selectedToken, wallet.account, CONTRACT_ADDRESSES.STAKING_REWARDS // This should be staking contract address
            );
            if (parseFloat(allowance) < stakeAmountNum) {
                showAlert('info', 'Approving token...', { absolute: true, position: 'top' });
                const approveTx = await blockchainActions.approveToken(selectedToken, CONTRACT_ADDRESSES.STAKING_REWARDS, // This should be staking contract address
                stakeAmount);
                showAlert('info', `Approval submitted: ${approveTx.hash.slice(0, 10)}...`, { absolute: true, position: 'top' });
                await approveTx.wait();
                showAlert('success', 'Token approved successfully', { absolute: true, position: 'top' });
    ...
    return (jsxRuntimeExports.jsx(Container, { ...containerProps, children: jsxRuntimeExports.jsxs("div", { className: "card", children: [jsxRuntimeExports.jsxs("div", { className: "cardHeader", children: [jsxRuntimeExports.jsx("h2", { className: "cardTitle", children: "Staking" }), jsxRuntimeExports.jsx("p", { className: "cardDescription", children: "Stake your tokens to earn rewards" })] }), jsxRuntimeExports.jsxs("div", { className: "cardContent", children: [jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "Select Token" }), jsxRuntimeExports.jsx("select", { value: selectedToken, onChange: (e) => setSelectedToken(e.target.value), disabled: isProcessing, className: "sel_field", children: supportedTokens.map(token => (jsxRuntimeExports.jsxs("option", { value: token, children: [TOKENS[token].name, " (", token, ")"] }, token))) })] }), jsxRuntimeExports.jsx("div", { className: "bg-gray-50 rounded-lg p-3 mb-4", children: jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm", children: [jsxRuntimeExports.jsxs("div", { className: "text-center ", children: [jsxRuntimeExports.jsx("span", { className: "text-gray-600", children: "Available:" }), jsxRuntimeExports.jsx("div", { className: "font-semibold text-blue-600", children: formatBalance(tokenBalance) })] }), jsxRuntimeExports.jsxs("div", { className: "text-center", children: [jsxRuntimeExports.jsx("span", { className: "text-gray-600", children: "Staked:" }), jsxRuntimeExports.jsx("div", { className: "font-semibold text-green-600", children: formatBalance(stakedBalance) })] }), jsxRuntimeExports.jsxs("div", { className: "text-center", children: [jsxRuntimeExports.jsx("span", { className: "text-gray-600", children: "Rewards:" }), jsxRuntimeExports.jsx("div", { className: "font-semibold text-purple-600", children: formatBalance(pendingRewards) })] })] }) }), jsxRuntimeExports.jsxs("div", { className: "bg-yellow-50 rounded-lg p-2 mb-4 text-center", children: [jsxRuntimeExports.jsx("span", { className: "text-sm text-yellow-600", children: "APY: " }), jsxRuntimeExports.jsxs("span", { className: "font-bold text-yellow-900", children: [apy, "%"] })] }), jsxRuntimeExports.jsxs("div", { className: "flex mb-4 bg-gray-100 rounded-lg p-1", children: [jsxRuntimeExports.jsx("button", { onClick: () => setActiveTab('stake'), className: `flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${activeTab === 'stake'
                                        ? 'bg-white text-blue-600 shadow-sm'
                                        : 'text-gray-600 hover:text-gray-900'}`, children: "Stake" }), jsxRuntimeExports.jsx("button", { onClick: () => setActiveTab('unstake'), className: `flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${activeTab === 'unstake'
                                        ? 'bg-white text-red-600 shadow-sm'
                                        : 'text-gray-600 hover:text-gray-900'}`, children: "Unstake" })] }), activeTab === 'stake' ? (jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center mb-2", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "Amount to Stake" }), jsxRuntimeExports.jsx("button", { type: "button", onClick: handleMaxStakeClick, disabled: isProcessing, className: "text-blue-500 hover:text-blue-600 font-medium text-sm", children: "MAX" })] }), jsxRuntimeExports.jsx("input", { type: "text", placeholder: "0.0", value: stakeAmount, onChange: (e) => setStakeAmount(e.target.value), disabled: isProcessing, className: "txt_field" }), jsxRuntimeExports.jsx("button", { onClick: handleStake, disabled: isProcessing || !isValidStakeAmount(), className: "actionButton mt-2", children: isProcessing ? (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx("div", { className: "loading-spinner" }), "Staking..."] })) : (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx(Coins, { className: "w-4 h-4" }), "Stake ", selectedToken] })) })] })) : (jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsxs("div", { className: "flex justify-between items-center mb-2", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "Amount to Unstake" }), jsxRuntimeExports.jsx("button", { type: "button", onClick: handleMaxUnstakeClick, disabled: isProcessing, className: "text-red-500 hover:text-red-600 font-medium text-sm", children: "MAX" })] }), jsxRuntimeExports.jsx("input", { type: "text", placeholder: "0.0", value: unstakeAmount, onChange: (e) => setUnstakeAmount(e.target.value), disabled: isProcessing, className: "txt_field" }), jsxRuntimeExports.jsx("button", { onClick: handleUnstake, disabled: isProcessing || !isValidUnstakeAmount(), className: "actionButton mt-2 bg-red-500 hover:bg-red-600", children: isProcessing ? (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx("div", { className: "loading-spinner" }), "Unstaking..."] })) : (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx(TrendingUp, { className: "w-4 h-4" }), "Unstake ", selectedToken] })) })] }))] }), jsxRuntimeExports.jsx(Alert, { ...alertProps })] }) }));
}

function Faucet({ onSuccess, onError, supportedTokens = ['USDT', 'USDC', 'WETH', 'DAI'], ...containerProps }) {
    const { wallet } = useWallet();
    const { showAlert, alertProps } = useAlert();
    ...
    const handleRequestTokens = async (token) => {
        if (!wallet.isConnected) {
            showAlert('error', 'Please connect your wallet first', { absolute: true, position: 'top' });
            return;
        }
    ...
    return (jsxRuntimeExports.jsx(Container, { ...containerProps, children: jsxRuntimeExports.jsxs("div", { className: "card", children: [jsxRuntimeExports.jsxs("div", { className: "cardHeader", children: [jsxRuntimeExports.jsx("h2", { className: "cardTitle", children: "Faucet" }), jsxRuntimeExports.jsx("p", { className: "cardDescription", children: "Get testnet tokens for development" })] }), jsxRuntimeExports.jsxs("div", { className: "cardContent", children: [jsxRuntimeExports.jsxs("div", { className: "inputGroup", children: [jsxRuntimeExports.jsx("label", { className: "label", children: "Select Token" }), jsxRuntimeExports.jsx("select", { value: selectedToken, onChange: (e) => setSelectedToken(e.target.value), disabled: isProcessing, className: "sel_field", children: supportedTokens.map(token => (jsxRuntimeExports.jsxs("option", { value: token, children: [TOKENS[token].name, " (", token, ")"] }, token))) })] }), faucetInfo[selectedToken] && (jsxRuntimeExports.jsx("div", { className: "bg-gray-50 rounded-lg p-3 mb-4", children: jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3 text-sm", children: [jsxRuntimeExports.jsxs("div", { className: "text-center", children: [jsxRuntimeExports.jsx("span", { className: "text-gray-600", children: "Your Balance:" }), jsxRuntimeExports.jsx("div", { className: "font-semibold text-blue-600", children: formatBalance(userBalances[selectedToken] || '0') })] }), jsxRuntimeExports.jsxs("div", { className: "text-center", children: [jsxRuntimeExports.jsx("span", { className: "text-gray-600", children: "Drip Amount:" }), jsxRuntimeExports.jsx("div", { className: "font-semibold text-green-600", children: formatBalance(faucetInfo[selectedToken]?.dripAmount || '0') })] }), jsxRuntimeExports.jsxs("div", { className: "text-center", children: [jsxRuntimeExports.jsx("span", { className: "text-gray-600", children: "Max Balance:" }), jsxRuntimeExports.jsx("div", { className: "font-semibold text-purple-600", children: formatBalance(faucetInfo[selectedToken]?.maxBalance || '0') })] }), jsxRuntimeExports.jsxs("div", { className: "text-center", children: [jsxRuntimeExports.jsx("span", { className: "text-gray-600", children: "Next Request:" }), jsxRuntimeExports.jsx("div", { className: "font-semibold text-orange-600", children: formatTimeRemaining(faucetInfo[selectedToken]?.timeUntilNextDrip || 0) })] })] }) })), faucetInfo[selectedToken] && (jsxRuntimeExports.jsx("div", { className: `rounded-lg p-2 mb-4 text-center text-sm ${getTokenStatus(selectedToken).color === 'green' ? 'bg-green-100 text-green-700' :
                                getTokenStatus(selectedToken).color === 'yellow' ? 'bg-yellow-100 text-yellow-700' :
                                    getTokenStatus(selectedToken).color === 'red' ? 'bg-red-100 text-red-700' :
                                        'bg-gray-100 text-gray-700'}`, children: jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-center gap-2", children: [getTokenStatus(selectedToken).status === 'available' && jsxRuntimeExports.jsx(CheckCircle, { className: "w-4 h-4" }), getTokenStatus(selectedToken).status === 'cooldown' && jsxRuntimeExports.jsx(Clock, { className: "w-4 h-4" }), jsxRuntimeExports.jsx("span", { className: "capitalize font-medium", children: getTokenStatus(selectedToken).status === 'available' ? 'Ready to claim' :
                                            getTokenStatus(selectedToken).status === 'cooldown' ? 'On cooldown' :
                                                getTokenStatus(selectedToken).status === 'inactive' ? 'Faucet inactive' :
                                                    'Loading...' })] }) })), jsxRuntimeExports.jsx("button", { onClick: () => handleRequestTokens(selectedToken), disabled: isProcessing ||
                                !wallet.isConnected ||
                                !faucetInfo[selectedToken] ||
                                !faucetInfo[selectedToken]?.isActive ||
                                !faucetInfo[selectedToken]?.canDrip ||
                                parseFloat(userBalances[selectedToken] || '0') >= parseFloat(faucetInfo[selectedToken]?.maxBalance || '0'), className: "actionButton w-full", children: isProcessing ? (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx("div", { className: "loading-spinner" }), "Processing..."] })) : !faucetInfo[selectedToken] ? ('Loading...') : !faucetInfo[selectedToken]?.isActive ? ('Faucet Inactive') : !faucetInfo[selectedToken]?.canDrip ? (`Wait ${formatTimeRemaining(faucetInfo[selectedToken]?.timeUntilNextDrip || 0)}`) : parseFloat(userBalances[selectedToken] || '0') >= parseFloat(faucetInfo[selectedToken]?.maxBalance || '0') ? ('Max Balance Reached') : (jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [jsxRuntimeExports.jsx(Droplets, { className: "w-4 h-4" }), "Request ", selectedToken] })) }), jsxRuntimeExports.jsx("div", { className: "mt-4 p-3 bg-blue-50 rounded-lg", children: jsxRuntimeExports.jsx("p", { className: "text-sm text-blue-700 text-center", children: "Testnet tokens \u2022 Daily limits apply \u2022 No real value" }) })] }), jsxRuntimeExports.jsx(Alert, { ...alertProps })] }) }));
}

export { Alert, BRIDGE_ABI, BlockchainActions, Bridge, CONTRACT_ADDRESSES, ConnectWallet, Container, ERC20_ABI, FAUCET_ABI, Faucet, HyperkitProvider, LIQUIDITY_POOL_ABI, NETWORKS, STAKING_ABI, Staking, Swap, TOKENS, TOKEN_ADDRESSES, createBlockchainActions, useAlert, useWallet };
